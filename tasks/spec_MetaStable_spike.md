...
## 1. Frame
A meta-stablecoin (call it **metaUSD**) **collateralized by stable LP positions** held in
...
- **Productive collateral** — the locked LP keeps earning V4 swap fees while it backs metaUSD. The same
...
- **Minter** — a stable-LP **position holder** (a `StableLPManager` owner). A distinct protocol role:
  locks LP as collateral and mints metaUSD against it. **The minter keeps the LP fees on the locked
...
- **Holder / user** — anyone using the circulating metaUSD. In v1 has no on-chain redemption right
...
- **Mint / lock (`tokenize(salt, L)`, owner-only):** raise `encumbered[salt] += L`, then mint metaUSD via
...
- **Burn folded into `withdrawTo` (no separate redeem):** the liquidity-reducing path used by `withdrawTo`
  ...
  > matching metaUSD, lowering `encumbered[salt]` accordingly.

  So the only way to free locked collateral is to **retire (burn) metaUSD**. `withdrawTo` stays
  `onlyOwnerNFT`.
- **EIP-170:** the manager has ~604 B of headroom. Keep its additions minimal (one mapping + the guard +
  `tokenize`); the **metaUSD ERC-20 is an external contract** the manager mints/burns through. Even so the
  guard + `tokenize` may need a lever (lower `optimizer_runs`, a satellite accounting contract) — measure
  before committing.

## 4. Token denomination
One fungible metaUSD over multiple heterogeneous positions needs **uniform backing per token**, else
burning tokens meant for position A could free position B and dilute A's backing. Options:
- **Pro-rata basket (recommended):** metaUSD supply maps to the *whole* encumbered basket; freeing
...
- **Shared protocol-level metaUSD** across many minters/managers (a real stablecoin) vs a **per-manager**
  token — the protocol vision wants one shared metaUSD backed by the aggregate of all minters' collateral.

## 5. Peg & redemption — the central open problem
What holds metaUSD at $1?
- **Backing:** ~par stable collateral ⇒ intrinsic value ≈ $1, and a minter is economically forced to buy
  metaUSD back (at ≤ backing) to unlock their collateral → a natural price floor + arbitrage.
- **v1 gap:** with owner-only `withdrawTo` there is **no holder-side force-redeem** — a holder trusts the
  ...
  **holder redemption path** (burn metaUSD → pull collateral pro-rata, in-kind, autonomous) and/or
...
- **Inherited risks:** depeg of an underlying stable; the manager's existing assumptions carry over
  ...
  (managers/pools) backs the shared metaUSD.
...
- Shared protocol-level metaUSD vs per-manager token; who governs accepted collateral.
- Fee-accounting precision: metaUSD backing must not double-count the **already-skimmed 10% protocol fee**
...
## Recommendation
Treat metaUSD as a **separate protocol layer** built on top of `StableLPManager`, with only the small
