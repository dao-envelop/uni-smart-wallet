...
> **⚠️ Частично заменено v2 (`task_014_stable_lp_v2.md`).** В реализации больше нет единого `QUOTE`,
> трёх фиксированных пулов и весов. Текущая модель: динамический набор произвольных стейбл-пулов
> (1..`MAX_POOLS`), депонировать можно любой управляемый стейбл, раскладку задаёт оператор «ногами»
> через `allocate` (авто: разворачивает баланс) или `allocateFrom` (ручной: указанный стейбл, с
> гардом «трогаем только его»). `withdrawTo` и инвариант непрямого вывода — без изменений.
> **Статус:** черновик архитектуры. Не реализовано.
> **Полная спека (EN):** `spec_StableLPManager.md`.
