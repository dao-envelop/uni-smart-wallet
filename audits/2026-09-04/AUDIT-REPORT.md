...
|---|---|---|---|
| **H-1** | **HIGH** | **ПОДТВ. PoC** | Скомпрометированный оператор выводит principal **без единого свопа**: swapless `recenter`/`moveLiquidity` кладут ликвидность по искажённой spot-цене; `moveLiquidity` позволяет выбрать самый тонкий пул из конфига |
