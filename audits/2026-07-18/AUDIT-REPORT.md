...
1. `recenter` вызывается оператором (`onlyAuthorized`). Он снимает **всю** ликвидность позиции:
   ...
   **весь principal остаётся положительной `currencyDelta`** — расходуемой внутри того же unlock. Пулы
   ...
   > поправка к самой этой заметке (task_044): в её первой редакции тут стояло «[M-3] аудита 2026-05-17
   > остаётся открытым» — это неверно и противоречило строке про Reentrancy ниже в этом же документе,
   > где уже сказано «v4 реверт вложенного `unlock`». M-3 **недостижим** на нашем пине v4 и закрыт как
   > NOT APPLICABLE; более того, рекомендованный им фикс (`nonReentrant` на `unlockCallback`) сломал бы
   > каждую операцию. Разбор — в блоке RESOLUTION у [M-3] в `audits/2026-05-17/AUDIT-REPORT.md`. Для
   > `OpenVolatileLPManager` остаточный риск — про дельту (H-6), а не про реентранси.
